...
let galaxyRotationSpeed = 0.00001; // Very slow base rotation speed
let realScale = false;
...
let activeCameraTween = null;  
 ...
 function updatePlanetPositions(deltaTime) {
    ...
    Object.keys(planets).forEach(planetId => {
        ...
        // Calculate the angle based on accumulated time and orbit speed
        const angle = accumulatedTime * data.orbitSpeed;
    ...
    Object.keys(moons).forEach(moonId => {
        ...
        if (planets[parentPlanetId]) {
            ...
            // Calculate angle based on accumulated time and moon's orbit speed
            const moonAngle = accumulatedTime * moonOrbitSpeed;
 ...
 function animate() {
    ...
    if (cameraFollowing && selectedPlanet && planets[selectedPlanet] && !mainMenuActive) {
        ...
        const planet = planets[selectedPlanet] || moons[selectedPlanet];
        const planetId = selectedPlanet;
        
        // Always point at the planet
        cameraTarget.copy(planet.position);
        
        // Use a higher lerp factor to ensure camera keeps up with fast-moving planets
        // 0.2 allows for smoother following without falling behind
        controls.target.lerp(cameraTarget, 0.2);
        
        // Update camera position to maintain relative position to the planet while it moves
        if (!activeCameraTween) {
            let offset;
            let lookAtSunFactor = 0.3; // How much to factor in the sun's position (center of system)
            
            if (planetId === 'moon' && moons[planetId]) {
                // Special handling for the moon
                const moon = moons[planetId];
                // Create a vector that points from moon to Earth
                const directionToEarth = new THREE.Vector3().subVectors(planets.earth.position, moon.position).normalize();
                const distance = realScale ? 0.3 : 1.5;
                
                offset = new THREE.Vector3().copy(directionToEarth).multiplyScalar(-distance);
                offset.y = distance * 0.3;
                
                // For moon, we want to see Earth too, so we'll orient differently
                lookAtSunFactor = 0.1;
            } else if (planetId === 'sun') {
                // Special handling for the sun - Sun is at center, so we just need an offset
                const data = orbitalData[planetId];
                const planetSize = realScale ? data.realSize / 10000 : data.size;
                const zoomFactor = 1.2;
                const distance = planetSize * zoomFactor * 5;
                
                offset = new THREE.Vector3(distance * 0.8, distance * 0.6, distance * 0.8);
                lookAtSunFactor = 0; // Sun is already at center, no need to factor it in
            } else if (planetId === 'pluto') {
                // Special handling for Pluto
                // Always point towards the sun from Pluto's position
                const directionToSun = new THREE.Vector3().subVectors(new THREE.Vector3(0, 0, 0), planet.position).normalize();
                const distance = 5;
                
                offset = new THREE.Vector3().copy(directionToSun).multiplyScalar(-distance);
                offset.y = distance * 0.3;
                lookAtSunFactor = 0.4; // Higher factor to see more of the system
            } else {
                ...
                const planetSize = realScale ? data.realSize / 10000 : data.size;
                const distance = planetSize * zoomFactor * 10;
                ...
                // Adjust lookAtSunFactor based on planet distance from sun
                // Outer planets need more focus on the center
            ...
            const targetCameraPosition = new THREE.Vector3().copy(planet.position).add(offset);
            
            // Adjust camera position with higher lerp factor (0.15) to keep up with fast movements
            camera.position.lerp(targetCameraPosition, 0.15);
            
            // Create a blended look-at point between the planet and the sun
            // This keeps the view somewhat locked to the center of the solar system
            if (lookAtSunFactor > 0) {
                const sunPosition = new THREE.Vector3(0, 0, 0); // Center of solar system
                const blendedTarget = new THREE.Vector3().copy(planet.position).lerp(sunPosition, lookAtSunFactor);
                controls.target.lerp(blendedTarget, 0.1);
            }
        }
...
function updateStarBrightness() {
 ...
 function updateGalaxyRotations(deltaTime) {
    ...
    if (homeGalaxy) {
        homeGalaxy.rotation.y += homeGalaxy.userData.rotationSpeed * smoothTimeSpeed * deltaTime;
    }
    
    // Update other galaxies rotation
    galaxies.forEach(galaxy => {
        if (galaxy && galaxy.userData && galaxy.userData.rotationSpeed) {
            galaxy.rotation.y += galaxy.userData.rotationSpeed * smoothTimeSpeed * deltaTime;
        }
    });
    ...
    galaxyClusters.forEach(cluster => {
        if (cluster && cluster.userData) {
            ...
            cluster.rotation.y += cluster.userData.rotationSpeed * smoothTimeSpeed * deltaTime;
        }
    });
}
